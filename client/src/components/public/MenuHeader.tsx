...
export default function MenuHeader({ vendor }: { vendor: Vendor }) {
  ...
  return (
    <div className="bg-black px-4 pt-6 pb-4">
      ...
      {chips.length > 0 && (
        <div className="flex mt-4 rounded-xl border border-neutral-800 divide-x divide-neutral-800 overflow-hidden">
          {chips.map(({ icon: Icon, label }, idx) => (
            <div key={idx} className="flex-1 flex flex-col items-center justify-center gap-1 py-2.5 px-1 text-center min-w-0">
              ...
              <span className="text-xs text-neutral-300 leading-tight truncate w-full">{label}</span>
            </div>
          ))}
        </div>
      )}
